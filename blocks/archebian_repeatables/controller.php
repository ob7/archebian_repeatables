<?php
namespace Concrete\Package\ArchebianRepeatables\Block\ArchebianRepeatables;
...
{
    protected $btTable = 'btArchebianRepeatables';
    protected $btExportTables = array('btArchebianRepeatables', 'btArchebianRepeatablesItem');
    ...
    public function getSearchableContent()
    {
        ...
        $db = Database::get();
        $v = array($this->bID);
        $q = 'select * from btArchebianRepeatablesItem where bID = ?';
        $r = $db->query($q, $v);
        foreach ($r as $row) {
            $content .= $row['title'].' ';
    ...
    public function edit()
    {
        ...
        $this->requireAsset('redactor');
        $db = Database::get();
        $query = $db->GetAll('SELECT * from btArchebianRepeatablesItem WHERE bID = ? ORDER BY sortOrder', array($this->bID));
        $this->set('rows', $query);
    }
    ...
    {
        $db = Database::get();
        $r = $db->GetAll('SELECT * from btArchebianRepeatablesItem WHERE bID = ? ORDER BY sortOrder', array($this->bID));
    ...
    public function duplicate($newBID)
    {
        ...
        while ($row = $r->FetchRow()) {
            $db->execute('INSERT INTO btArchebianRepeatablesItem (bID, fID, linkURL, title, description, sortOrder, internalLinkCID) values(?,?,?,?,?,?,?)',
                array(
                    $newBID,
                    $row['fID'],
                    $row['linkURL'],
    ...
    {
        $db = Database::get();
        $db->delete('btArchebianRepeatablesItem', array('bID' => $this->bID));
        parent::delete();
    }
    ...
    public function save($args)
    {
        ...
        $db = Database::get();
        $db->execute('DELETE from btArchebianRepeatablesItem WHERE bID = ?', array($this->bID));
        parent::save($args);
        if (isset($args['sortOrder'])) {
            ...
            while ($i < $count) {
                ...
                $db->execute('INSERT INTO btArchebianRepeatablesItem (bID, fID, title, description, sortOrder, linkURL, internalLinkCID) values(?, ?, ?, ?,?,?,?)',
                    array(
                        $this->bID,
                        intval($args['fID'][$i]),
                        $args['title'][$i],
